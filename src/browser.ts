...
async function convertWithCanvas(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      if (!ctx)
        return reject(new ConversionError('Failed to get canvas context'));

      ctx.drawImage(img, 0, 0, targetWidth, targetHeight);
      canvas.toBlob(
        (resultBlob) => {
          if (!resultBlob)
            return reject(new ConversionError('Failed to convert to WebP'));
          resolve({
            blob: resultBlob,
            width: targetWidth,
            height: targetHeight,
...
export async function convertToWebP(
  input: BrowserInput,
  options?: ConvertOptions
): Promise<ConvertResult<Blob>> {
  if (!(input instanceof File) && !(input instanceof Blob)) {
    throw new InvalidInputError(
      'Input must be File or Blob in browser',
      typeof input
    );
  }
