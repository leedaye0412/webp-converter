...
export function validateQuality(quality: unknown): asserts quality is number {
  ...
  if (!Number.isInteger(quality)) {
    throw new InvalidOptionsError("quality", quality, `integer between ${QUALITY_MIN} and ${QUALITY_MAX}`)
  }
  if (quality < QUALITY_MIN || quality > QUALITY_MAX) {
    throw new InvalidOptionsError("quality", quality, `number between ${QUALITY_MIN} and ${QUALITY_MAX}`)
  }
}
...
export function validateResizeOptions(options: unknown): asserts options is ResizeOptions {
  ...
  if (resizeOpts["width"] !== undefined) {
    if (typeof resizeOpts["width"] !== "number" || resizeOpts["width"] <= 0) {
      throw new InvalidOptionsError("resize.width", resizeOpts["width"], "positive number")
    }
  }

  if (resizeOpts["height"] !== undefined) {
    if (typeof resizeOpts["height"] !== "number" || resizeOpts["height"] <= 0) {
      throw new InvalidOptionsError("resize.height", resizeOpts["height"], "positive number")
    }
  }
  ...
  if (resizeOpts["fit"] !== undefined) {
    ...
    if (!validFits.includes(resizeOpts["fit"] as string)) {
      throw new InvalidOptionsError("resize.fit", resizeOpts["fit"], `one of: ${validFits.join(", ")}`)
    }
  }
}

export function validateConvertOptions(options: unknown): asserts options is ConvertOptions {
  ...
  if (opts["preserveMetadata"] !== undefined && typeof opts["preserveMetadata"] !== "boolean") {
    throw new InvalidOptionsError("preserveMetadata", opts["preserveMetadata"], "boolean")
  }
  if (opts["method"] !== undefined) {
    const validMethods = ["default", "fast", "best"]
    if (!validMethods.includes(opts["method"] as string)) {
      throw new InvalidOptionsError("method", opts["method"], `one of: ${validMethods.join(", ")}`)
    }
  }
}

export function detectFormatFromExtension(filename: string): SupportedInputFormat | null {
