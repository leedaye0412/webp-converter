  ...
  try {
    ...
    if ((error as NodeJS.ErrnoException).code === 'MODULE_NOT_FOUND') {
      throw new ConversionError(
        'Sharp is required. Install: npm install sharp',
        error as Error
      );
    }
    throw new ConversionError(
...
export function createConverter(defaultOptions?: ConvertOptions) {
  ...
  return {
    convert: (input: NodeInput, options?: ConvertOptions) =>
      convertToWebP(input, { ...baseOptions, ...options }),
