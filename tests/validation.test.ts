...
import { validateQuality, validateResizeOptions, detectFormatFromExtension } from "../src/utils/validation.js"
...
describe("validateResizeOptions", () => {
  it("should accept valid resize options", () => {
    ...
    expect(() => validateResizeOptions({ width: 100, height: 200, fit: "cover" })).not.toThrow()
  })

  it("should reject invalid options", () => {
    expect(() => validateResizeOptions("100x200")).toThrow(InvalidOptionsError)
    expect(() => validateResizeOptions({ width: -100 })).toThrow(InvalidOptionsError)
  })
})
